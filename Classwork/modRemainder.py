...
def draw():
    ...
    circle(0, 0, 200)
    stroke_weight(2)
    hourRad = remap(hour() % 12 + minute() / 60.0, 0, 12, 0, TWO_PI)
    minRad  = remap(minute() + second() / 60.0, 0, 60, 0, TWO_PI)
    ...
    for i in range(12):
        ...
        fill(0)
        numRad = remap(i, 0, 12, 0, TWO_PI) - PI / 3
        num = i + 1
        if num == 13:
            num = 1
        text(str(num), 75 * cos(numRad), 75 * sin(numRad))
        fill(255)
    pop_matrix()